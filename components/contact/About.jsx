...
const FALLBACK_IMAGE = "/images/section/section-contact-2.jpg";
...
export default function About() {
  ...
  const title =
    (c.contactAboutTitle && String(c.contactAboutTitle).trim()) ||
    DEFAULT_TITLE;
  const subtitle =
    (c.contactAboutSubtitle && String(c.contactAboutSubtitle).trim()) ||
    DEFAULT_SUBTITLE;

  const address = (c.address || "").trim();
  const phones = (c.phones || []).map((p) => String(p).trim()).filter(Boolean);
  const emails = (c.emails || []).map((e) => String(e).trim()).filter(Boolean);
  const hours = (c.workingHours || "").trim();

  const imageSrc =
    (c.contactPageImage && String(c.contactPageImage).trim()) || FALLBACK_IMAGE;

  return (
    <section
      className="section-contact style-4 tf-spacing-1 pb-0"
    ...
    >
      <div className="tf-container">
        <div className="row">
              ...
              <ul className="list-info">
                  ...
                  <li>
                    <div className="icon">
                      <svg
                        ...
                        aria-hidden
                      >
                        <g clipPath="url(#clip_contact_mail)">
                          <path
                ...
                {hours ? (
                  <li>
                    ...
                    <div className="content">
                      <div className="sub">Working hours</div>
                      <p className="mb-0" style={{ whiteSpace: "pre-line" }}>
                        {hours}
                      </p>
                    </div>
          ...
          <div className="col-md-6">
            <div className="image-wrap contact-about-image-wrap">
              <Image
                className="lazyload"
                src={imageSrc}
                alt=""
